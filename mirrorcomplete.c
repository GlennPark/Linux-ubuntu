...
//		index = (height-i-1) *size; 
		...
		index = (width-j-1)*3;
		   ...
		   outimg[i*size+index+0] = inimg[size*i+3*j+0];
           outimg[i*size+index+1] = inimg[size*i+3*j+1];
		   outimg[i*size+index+2] = inimg[size*i+3*j+2];

//			outimg[i+*index+0] = inimg[i+3*size+0];
//			outimg[i+*index+1] = inimg[i+3*size+1];
//			outimg[i+3*index+2] = inimg[i+3*size+2];
		};
	};
